 ...
 * @time 2016-09-25 20:42:54 options 参数说明 ------------------
 * @param title
 *            String 头部提示
 * @param prov
 *            String 默认省份
 ...
 *            String 默认区/县
 * @param selectNumber
 *            Int 设置省市县可选择个数 1-2,默认为3个都可选
 * @param scrollToCenter
 *            Boolean true:已选位置滚动到中央 ,默认为顶部
 * @param autoOpen
 *            Boolean 自动打开
 * @param customOutput
 *            Function 自定义选择完毕输出，不执行内部填充函数
 * @param selectEnd
 *            Function 选择完毕回调事件 return {prov,city,district},address
 *            ------------------
 * @version 1.0.0
 */
(function(factory) {
	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($) {
			...
			function address(elm, options) {
				...
				self.$popup.on('close.modal.amui', function() {
					...
					if (selectProv != self.options.prov) {
						self.$prov.find(
								"li:contains(" + self.options.prov + ")")
								.addClass("am-active").siblings().removeClass(
										"am-active")
					}
				});
			...
			address.prototype._ready = function() {
				var self = this;
				var html = [];
				html.push('<div style="z-index:1500 !important;" class="am-popup am-address-popup" id="'
						+ Math.ceil(Math.random() * (new Date).getTime())
						+ '">');
				html.push('<div class="am-popup-inner">');
				html.push('<div class="am-popup-hd">');
				html.push('<h4 class="am-popup-title">' + self.options.title
						+ '</h4>');
				html
						.push('<span data-am-modal-close class="am-close">&times;</span>');
				html.push('</div>');
				html.push('<div class="am-popup-bd">');
				if (self.options.footer) {
					html
							.push('<div class="am-address-list am-address-list-footer">')
				} else {
					html.push('<div class="am-address-list">')
				}
				html
						.push('<div class="am-u-sm-4 am-address-prov"><ul class="am-list am-list-static"></div>');
				html
						.push('<div class="am-u-sm-4 am-address-city"><ul class="am-list am-list-static"></div>');
				html
						.push('<div class="am-u-sm-4 am-address-district"><ul class="am-list am-list-static"></div>');
				html.push('</div>');
				if (self.options.footer) {
					html
							.push('<div class="am-modal-footer am-address-footer">');
					html
							.push('<span class="am-modal-btn am-address-cancel">取消</span>');
					html
							.push('<span class="am-modal-btn am-address-confirm">确定</span>');
					html.push('</div>')
				}
				html.push('</div>');
				html.push('</div>');
				...
				if (self.options.selectNumber == 1) {
					self.$popup.find("div.am-address-list>div").removeClass(
							"am-u-sm-4").addClass("am-u-sm-12")
				} else {
					self._readyCity("create")
				}
				if (self.options.selectNumber == 2) {
					self.$popup.find("div.am-address-list>div").removeClass(
							"am-u-sm-4").addClass("am-u-sm-6")
				} else {
			...
			address.prototype._readyProv = function(methods) {
				var self = this;
				if (methods == "create") {
					var prov = address._readyList(addressJson,
							self.options.prov);
					self.$prov.children("ul").html(prov.list).data(
							"address-active", prov.index);
					if (!prov.index) {
						self.$prov.find("li:eq(0)").addClass("am-active")
								.siblings("li").removeClass("am-active")
					}
					...
					self.$prov.on("addressTap", "li", function() {
						...
						$this.addClass("am-active").siblings("li").removeClass(
								"am-active");
						if (self.options.selectNumber != 1) {
					...
					self.provIscroll.scrollToElement(self.$prov
							.find("li.am-active")[0], null, null,
			...
			address.prototype._readyCity = function(methods, onEvent) {
				...
				if (methods == "create") {
					...
					var city = address._readyList(addressJson[provIndex].city,
							self.options.city);
					self.$city.children("ul").html(city.list).data(
							"address-active", city.index);
					if (!city.index) {
						self.$city.find("li:eq(0)").addClass("am-active")
								.siblings("li").removeClass("am-active")
					}
					if (onEvent === "tap"
							&& addressJson[provIndex].city.length <= 1
							&& self.options.selectNumber == 2) {
						self._selectEnd()
					}
					self.cityIscroll.refresh();
					self.cityIscroll.scrollToElement(self.$city
							.find("li.am-active")[0], null, null,
							self.options.scrollToCenter);
					if (self.options.selectNumber != 2) {
						self._readyDistrict()
			...
			address.prototype._readyDistrict = function(methods, onEvent) {
				...
				if (methods == "create") {
					...
					var district = address._readyList(districtJson,
							self.options.district);
					self.$district.children("ul").html(district.list).data(
							"address-active", district.index);
					if (!district.index) {
						self.$district.find("li:eq(0)").addClass("am-active")
								.siblings("li").removeClass("am-active")
					}
					self.districtIscroll.refresh();
					self.districtIscroll.scrollToElement(self.$district
							.find("li.am-active")[0], null, null,
							self.options.scrollToCenter)
				}
			};
			...
			$.AMUI.ready(function(doc) {
				$('[data-am-address]', doc).each(
						function() {
							var $this = $(this);
							var options = $.AMUI.utils.parseOptions($this
									.attr('data-am-address')
